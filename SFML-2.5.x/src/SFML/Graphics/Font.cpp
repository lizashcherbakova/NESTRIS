...
bool Font::loadFromFile(const std::string& filename)
{
    ...
    FT_Error error = FT_New_Face(static_cast<FT_Library>(m_library), filename.c_str(), 0, &face);
    if (error != 0)
    {
      std::string errorString;
      switch(error) {
        case FT_Err_Unknown_File_Format:
          errorString = "The file format is not supported.";
          break;
        case FT_Err_Cannot_Open_Resource:
          errorString = "The file could not be opened.";
          break;
          // Add more cases here based on the error codes defined in fterrdef.h
        default:
          errorString = "An unknown error occurred.";
          break;
      }
      err() << "Failed to load font \"" << filename << "\" (failed to create the font face)"
            << "FreeType Error: " << errorString << std::endl;
      return false;
    }
